...
export function Candidates() {
  ...
  const [candidate, setCanidate] = useState<Candidate>({
    firstName: "",
    lastName: "",
    number: "",
    photo: "",
  ...
  const openDialoag = (candidate: Candidate) => {
    setIsOpen(true)
    setCanidate(candidate)
  }
    ...
    <main>
      ...
      {Object.keys(groupedByRole).map(role => (
        ...
        px-[120px] mb-[130px]" key={role}>
          <h1 className="text-[38px] mb-[49px]">CANDIDATO <span className="font-bold">A {role.toUpperCase()}</span></h1>
          ...
          <DialogPanel className="bg-white rounded-[30px] shadow-md p-8 space-y-6 max-w-md w-full">
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col">
                <DialogTitle className="font-bold text-[20px]">{candidate.firstName} {candidate.lastName}</DialogTitle>
